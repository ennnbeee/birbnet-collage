...
Step 1 of the illustration pipeline:
    1. generate_images.py       render each bird on a uniform cream ground
    2. generate_cutout.py       remove the ground (BiRefNet) and crop to the bird
    3. generate_masks_dims.py  refresh the collage silhouette masks in apt.js
...
def load_config_keys(config_path: Path | None = None) -> dict[str, str]:
    """Load API keys from config.yaml if it exists. Returns dict with
    'gemini_api_key' and 'ebird_api_key' (empty strings if not found).
    """
    if not config_path:
        # Default to config.yaml in project root (two dirs up from scripts/)
        config_path = Path(__file__).resolve().parents[1] / "config.yaml"

    if not config_path.exists() or not yaml:
        return {"gemini_api_key": "", "ebird_api_key": ""}
    ...
    try:
        ...
        return {
            ...
            "ebird_api_key": api_keys.get("ebird_api_key", ""),
        }
    except Exception:
        return {"gemini_api_key": "", "ebird_api_key": ""}
...
def main() -> int:
    ...
    ap.add_argument("--ebird-key", help="eBird API key (or EBIRD_API_KEY env or config.yaml)")
    ap.add_argument("--gemini-key", help="Gemini API key (or GEMINI_API_KEY env or config.yaml)")
    ap.add_argument("--config", type=Path, help="Path to config.yaml for API keys (default: ../config.yaml)")
    ...
    # Load API keys from config file, then override with env vars, then args
    config_keys = load_config_keys(args.config if hasattr(args, 'config') else None)
    gemini_key = (
        args.gemini_key
        or os.environ.get("GEMINI_API_KEY", "")
        or config_keys.get("gemini_api_key", "")
    )
    if not gemini_key:
        print("error: GEMINI_API_KEY required (--gemini-key, env, or config.yaml)", file=sys.stderr)
        return 2

    # Build species list
    if args.labels:
        species, skipped = parse_species_list(args.labels.read_text(encoding='utf-8').splitlines())
    elif args.stdin:
    ...
    if args.ebird_region:
        ek = (
            ...
            or config_keys.get("ebird_api_key", "")
        )
        if not ek:
            print("error: --ebird-region requires --ebird-key, EBIRD_API_KEY env, or config.yaml", file=sys.stderr)
            return 2
    ...
    for idx, (sci, com) in enumerate(species):
        ...
        # Check if any poses need generation before fetching references
        needs_generation = args.force or any(
            not (args.out / (f"{slug}.png" if pose == 1 else f"{slug}-{pose}.png")).exists()
            for pose in args.poses
        )

        if not needs_generation:
            skipped_existing += len(args.poses)
            continue
